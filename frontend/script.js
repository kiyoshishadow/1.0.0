  ...
  try {
    ...
    if (cached) {
      try {
        ...
        usuarioRol = normalizarRol(u.rol);
        usuarioId = u.id || u.usuario_id;
        usuarioNombre = u.nombre;
        document.getElementById('usuario-actual').textContent = u.nombre;
        document.getElementById('rol-actual').textContent = normalizarRol(u.rol);
        configurarMenuPorRol();
        return true;
      } catch {
...
function configurarMenuPorRol() {
  document.querySelectorAll('.sidebar-link').forEach(link => {
    ...
    link.style.display = puedeAcceder(seccion) ? '' : 'none';
  });

  document.getElementById('btn-nueva-impresora').style.display = tieneRol('administrador', 'supervisor') ? '' : 'none';
  document.getElementById('btn-nuevo-suministro').style.display = tieneRol('administrador', 'supervisor') ? '' : 'none';
  document.getElementById('btn-nuevo-mantenimiento').style.display = tieneRol('administrador', 'supervisor', 'tecnico') ? '' : 'none';
  document.getElementById('btn-nuevo-registro').style.display = tieneRol('administrador', 'supervisor', 'operario') ? '' : 'none';
  document.getElementById('btn-nuevo-usuario').style.display = tieneRol('administrador') ? '' : 'none';
}

function mostrarSeccion(seccion) {
  document.querySelectorAll('.seccion-panel').forEach(p => p.classList.remove('active'));
  document.querySelectorAll('.sidebar-link').forEach(l => l.classList.remove('active'));
  document.getElementById(`panel-${seccion}`)?.classList.add('active');
...
function navegarDesdeResumen(seccion, filtro) {
  ...
  setTimeout(() => {
    if (seccion === 'impresoras') {
      const select = document.getElementById('filtro-estado');
      if (select) {
        if (filtro === 'activas') select.value = 'activo';
        else if (filtro === 'inactivas') select.value = 'inactivo';
        else if (filtro === 'mantenimiento') select.value = 'mantenimiento';
        select.dispatchEvent(new Event('change'));
      }
    } else if (seccion === 'suministros') {
      const select = document.getElementById('filtro-suministros');
      if (select && filtro === 'bajos') {
        select.value = 'bajo';
        select.dispatchEvent(new Event('change'));
      }
    } else if (seccion === 'mantenimientos') {
      const select = document.getElementById('filtro-mantenimiento');
      if (select && filtro === 'pendientes') {
